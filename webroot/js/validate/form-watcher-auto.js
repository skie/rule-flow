(function() {
    ...
    class ValidationRuleLoader {
        ...
        parseDataJsonLogicRules(form) {
            const dataJsonLogicRules = this.parseFormWithDataJsonLogic(form);
            const extractedRules = {};
    ...
    class FormWatcherAutoInitializer {
                ...
                if (window.FormWatcherCustomOperations) {
                    ...
                    Object.keys(customOps).forEach(opName => {
                        if (typeof customOps[opName] === 'function') {
                            ...
                            console.log(`FormWatcher: Registered custom operation "${opName}"`);
                        }
                    });
                }
            } catch (error) {
        ...
        registerCustomOperation(operationName, operationFunction) {
            ...
            try {
                ...
                console.log(`FormWatcher: Registered custom operation "${operationName}"`);
                return true;
            } catch (error) {
        ...
        initializeForm(form) {
            ...
            const validator = new ValidationRuleLoader();
        ...
        focusFirstErrorField(form, errors) {
            ...
            if (firstErrorField) {
                const inputField = form.querySelector(`[name="${firstErrorField}"]`);
                if (inputField) {
                    inputField.focus();
    ...
    function initializeFormWatcherAuto(options = {}) {
        if (typeof window === 'undefined') {
            return;
        }
        const initializer = new FormWatcherAutoInitializer(options);
        ...
        window.FormWatcherAuto = {
            ...
            configure: (newOptions) => {
                Object.assign(initializer.options, newOptions);
                return window.FormWatcherAuto;
            },
            getOptions: () => ({ ...initializer.options }),
            registerCustomFunction: (name, func) => initializer.registerCustomOperation(name, func)
        };

        const shouldAutoInit = window.FormWatcherAutoConfig?.autoInit !== false;
        ...
        if (shouldAutoInit) {
            ...
            console.log('FormWatcher Auto: Auto-initialization disabled. Call FormWatcherAuto.init() manually.');
        }

        return window.FormWatcherAuto;
    }

    const shouldInit = window.FormWatcherAutoConfig?.enabled !== false;
